...
prompt_command_execution_time() {
  ...
  if (( _P9K_COMMAND_DURATION >= POWERLEVEL9K_COMMAND_EXECUTION_TIME_THRESHOLD )); then
    "$1_prompt_segment" "$0" "$2" "" "226" "${humanReadableDuration}" 'EXECUTION_TIME_ICON'
  fi
}
...
prompt_dir() {
  ...
  "$1_prompt_segment" "$0_${current_state}" "$2" "" "blue" "${current_path}" "${dir_states[$current_state]}"
}
...
prompt_status() {
  if [[ "$RETVAL" -ne 0 ]]; then
    if [[ "$POWERLEVEL9K_STATUS_VERBOSE" == true ]]; then
      "$1_prompt_segment" "$0_ERROR" "$2" "" "226" "$RETVAL" 'CARRIAGE_RETURN_ICON'
    else
      "$1_prompt_segment" "$0_ERROR" "$2" "" "red" "" 'FAIL_ICON'
    fi
  elif [[ "$POWERLEVEL9K_STATUS_VERBOSE" == true || "$POWERLEVEL9K_STATUS_OK_IN_NON_VERBOSE" == true ]]; then
    "$1_prompt_segment" "$0_OK" "$2" "" "green" "" 'OK_ICON'
  fi
}
...
prompt_vcs() {
  ...
  if [[ -n "$vcs_prompt" ]]; then
    ...
    "$1_prompt_segment" "${0}_${(U)current_state}" "$2" "" "${vcs_states[$current_state]}" "$vcs_prompt" "$vcs_visual_identifier"
  fi
}
